...
2. **Clone your forked repository** to your local machine:
   ...
   ```bash
   git clone git remote set-url origin git@github.com:YOUR_USERNAME/REPOSITORY_NAME.git

   cd REPOSITORY_NAME
   ```
   Cloning this way only works because the repository we are working with is public. Forking a repository defaults to public and cannot be privated.

   If you were to clone a private repository, you would have to do so with a SSH Authentication -- we set this up in 216 -- or Personal Access Token 
...
3. **Add your answer to the first question** in `answers.txt`:
    you can use any command line text editor, if you're unsure of which to use vvv
...
4. **Stage and commit your changes**:
   First we want to "add" our changed files into the Staging area. The staging area is a place where you gather all your changes to then be committed all at once later. This means that our changes have not yet effected out Github repository
   ```bash
   git add answers.txt
   ```
...
### Step 3: Handle a Merge Conflict
...
### Step 1: Open answers.txt and copy/answer this question 
...
3. **Resolve any conflicts** 
   ...
   We didn't in this case, but if we did multiple different commits during our feature branch `q3`, we can use `--contue` to move to the next commit of out feature branch.
     ```bash
     git rebase --continue
     ```
...
4. **Force push** the rebased branch to your remote repository:
   Now we will push these changes to our remote repository. Usually, we can just `git push orgin [specify branch here/defaults to current branch if nothing]`, but if this doesnt work .  We can use the `-f` flag. This happeneds because rebase changes the commit history, and this will conflict with our exisiting commit history. The `-f` flag will allow us to "force" these changes without warning/errors from git.
...
1. **Submit a link to your github**
